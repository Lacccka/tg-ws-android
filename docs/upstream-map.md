...
| `proxy/tg_ws_proxy.py` | `_try_handshake`, `_generate_relay_init`, `_read_client_init`, `_build_crypto_ctx`, `_handle_client`, `_run`, `run_proxy` | future `proxy/TgWsProxy.kt`, `proxy/Handshake.kt`, `proxy/RelayInit.kt`, Android service entrypoint | Pending; do not implement networking yet. |
| `proxy/bridge.py` | `CryptoCtx`, `MsgSplitter`, fallback functions, TCP/WebSocket re-encryption bridge | future `proxy/crypto/CryptoContext.kt`, `proxy/bridge/MessageSplitter.kt`, `proxy/bridge/Bridge.kt`, `proxy/fallback/*` | Pending. |
| `proxy/raw_websocket.py` | `RawWebSocket`, `_xor_mask`, socket option setup, handshake errors | future `proxy/ws/RawWebSocket.kt`, `proxy/ws/WebSocketMask.kt` | Pending. |
...
`tools/check_upstream.py` before porting code changes.
